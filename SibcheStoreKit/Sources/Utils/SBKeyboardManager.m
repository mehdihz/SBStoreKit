...
static int _SBKeyboardViewFrameObserverKey;

/// Observer for view's frame/bounds/center/transform
@interface _SBKeyboardViewFrameObserver : NSObject
...
@implementation _SBKeyboardViewFrameObserver {
...
- (void)addToKeyboardView:(UIView *)keyboardView {
    ...
    if (_keyboardView) {
        ...
        objc_setAssociatedObject(_keyboardView, &_SBKeyboardViewFrameObserverKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _keyboardView = keyboardView;
    if (keyboardView) {
        [self addFrameObserver];
    }
    objc_setAssociatedObject(keyboardView, &_SBKeyboardViewFrameObserverKey, self, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
...
+ (instancetype)observerForView:(UIView *)keyboardView {
    ...
    return objc_getAssociatedObject(keyboardView, &_SBKeyboardViewFrameObserverKey);
}
...
@implementation SBKeyboardManager {
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"SBKeyboardManager init error" reason:@"Use 'defaultManager' to get instance." userInfo:nil];
    return [super init];
}
...
- (void)_initFrameObserver {
    ...
    _SBKeyboardViewFrameObserver *observer = [_SBKeyboardViewFrameObserver observerForView:keyboardView];
    if (!observer) {
        observer = [_SBKeyboardViewFrameObserver new];
...
- (void)_notifyAllObservers {
    ...
    if (!CGRectEqualToRect(trans.toFrame, _fromFrame)) {
        for (id<SBKeyboardObserver> observer in _observers.copy) {
